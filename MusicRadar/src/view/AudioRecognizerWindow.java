...
public class AudioRecognizerWindow extends JFrame {
	...
	double recordPoints[][];
	long points[][];
	//Map<Long, List<DataPoint>> hashMap;//存放hash数和对应的song_id和出现时间
	...
	private void listenSound(String filePath,int songId, boolean isMatching)
...
//						outFile.close();
	...
	void makeSpectrum(ByteArrayOutputStream out, int songId, boolean isMatching) throws SQLException {
...
//		spectrumView.setVisible(true);
	...
	// Find out in which range确定频率在哪个频段<40;40-80;80-120;120-180;180-300;300+
	...
	void determineKeyPoints(Complex[][] results, int songId, boolean isMatching) throws SQLException {
...
//			}
			...
			if (isMatching) {
				...
				//从hashMap里面得到h对应的list<DataPoint>,对于每一个数据点，求出其时间与当前时间的偏移量offset。然后，
				if ((listPoints = DBOpe.get_hashMap(h)) != null) {
...
//					DBOpe.add_hashMap_all(h, listPoints);
//				} else {
//					DataPoint point = new DataPoint((int) songId, t);
//					listPoints.add(point);
//					//DBOpe.add_hashMap(h, point);
//				}
...
//		}
	...
	public void createWindow() {
		...
		fileTextField.setText("E:\\CloudMusic\\");
		
		nrSong=DBOpe.Init_nrSong();//initialize songid
					...
					try {
						String filePath=fileTextField.getText();
						   ...
						   if(file.isDirectory()){
							...
							for(File f:files){
						    DBOpe.addsong(nrSong, f.getName());
							listenSound(f.getPath(),nrSong, false);
							nrSong++;;
							}
						   }else if(file.isFile()){
							  listenSound(file.getPath(), nrSong, false);
							  nrSong++;
						   }
					} catch (IOException e1) {
						e1.printStackTrace();
		...
		buttonMatch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int id:matchMap.keySet()) {
					...
					System.out.println("For song id: " + id +" name: "+DBOpe.getsongName(id));
					Map<Integer, Integer> tmpMap = matchMap.get(id);
					int bestCountForSong = 0;
                    int score=0;
					for (Map.Entry<Integer, Integer> entry : tmpMap.entrySet()) {//err
						if (entry.getValue() > bestCountForSong) {
							bestCountForSong = entry.getValue();
						}
						score+=entry.getValue()*entry.getValue();
...
//					}
				...
				System.out.println("Best song id: " + bestSong+" name: "+DBOpe.getsongName(bestSong)
				+ "\n" + " bestScore : " + bestScore);
			}
		});
